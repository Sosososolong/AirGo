...
$utilsProjDir = Join-Path -Path $currentDirectory -ChildPath ../Sylas.RemoteTasks.Database
Set-Location $utilsProjDir
dotnet build -c=Release
dotnet nuget push ./bin/Release/Sylas.RemoteTasks.Database.$version.nupkg -k $apikey -s https://api.nuget.org/v3/index.json --skip-duplicate
