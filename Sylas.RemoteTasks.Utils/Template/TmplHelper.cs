...
namespace Sylas.RemoteTasks.Utils.Template
{
    ...
    public static partial class TmplHelper
    {
        ...
        {
            LoggerHelper.WriteLog($"正在记录{sourceName}", "TmplResoving");
            if (source is string sourceStr)
            {
                LoggerHelper.WriteLog(sourceStr, "TmplResoving");
            }
            else if (source is IEnumerable sourceEnumerable)
            {
                ...
                if (sourceObjs.Any())
                {
                    ...
                    if (first is JsonElement)
                    {
                        ...
                        LoggerHelper.WriteLog($"集合每一项为JsonElement集合:", "TmplResoving");
                        StringBuilder itemBuilder = new();
                        foreach (var item in sourceJsonEles)
                        {
                            itemBuilder.AppendLine(item.ToString());
                        }
                        LoggerHelper.WriteLog($"[{itemBuilder}]", "TmplResoving");
                    }
                }
                else
                {
                    LoggerHelper.WriteLog(JsonConvert.SerializeObject(sourceObjs), "TmplResoving");
                }
            }
            else
            {
                LoggerHelper.WriteLog(source is JsonElement ? source.ToString() : JsonConvert.SerializeObject(source), "TmplResoving");
            }
        }
