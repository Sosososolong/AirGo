...
#pragma warning restore CS1570 // XML 注释出现 XML 格式错误
    {
        ...
        public static ParseResult ResolveCollectionSelectTmpl(string key, string prop, string recursively, Dictionary<string, object> dataContext)
        {
            ...
            if (keyVal is JsonElement keyValJE && keyValJE.ValueKind == JsonValueKind.Array)
            {
                var valueList = keyValJE.EnumerateArray();
                if (!valueList.Any())
                {
            ...
            else if (keyVal is not string && keyVal is IEnumerable valueCollection)
            {
