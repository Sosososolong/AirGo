...
## 更新摘要
**所做更改**
- 新增了SystemCmd的ANSI转义序列过滤功能说明
- 更新了SystemCmd组件详解部分，包含RemoveAnsiEscapeSequences方法
- 增强了故障排查指南，包含ANSI转义序列相关问题
- 更新了性能与并发特性部分，说明ANSI过滤对性能的影响
...
**最新更新** SystemCmd已添加ANSI转义序列过滤功能，确保在Web界面中显示干净的文本输出，去除颜色代码和格式化伪影。
...
- SystemCmd：系统命令执行器，支持单命令、批量命令、并行命令与主机信息采集，现已包含ANSI转义序列过滤功能。
...
- [FileHelper.cs:1-1559](file://Sylas.RemoteTasks.Utils/CommandExecutor/FileHelper.cs#L1-L1559)
- [SystemCmd.cs:1-986](file://Sylas.RemoteTasks.Utils/CommandExecutor/SystemCmd.cs#L1-L986)
...
**章节来源**
- [FileHelper.cs:27-1559](file://Sylas.RemoteTasks.Utils/CommandExecutor/FileHelper.cs#L27-L1559)
...
- **新增功能** ANSI转义序列过滤：
  - RemoveAnsiEscapeSequences方法：使用编译的正则表达式模式`\x1b\[[0-9;?]*[ -/]*[@-~]`移除终端控制码
  - 自动检测并清理颜色代码和格式化字符，确保在Web界面中显示干净文本
  - 应用于所有命令输出，包括标准输出和错误输出
- 使用建议：
  - 建议在 Windows 使用 PowerShell，Linux/macOS 使用 Bash。
  - 注意输出编码与临时文件清理，避免磁盘占用。
  - ANSI过滤功能会自动处理大多数终端颜色问题，无需额外配置。
...
ReadOut --> Filter["ANSI转义序列过滤"]
Filter --> CleanText["清理颜色代码和格式化字符"]
CleanText --> Yield["逐行产出 CommandResult"]
Yield --> Done(["结束"])
```
...
- [SystemCmd.cs:406-420](file://Sylas.RemoteTasks.Utils/CommandExecutor/SystemCmd.cs#L406-L420)
...
- SystemCmd：
  ...
  - **ANSI过滤性能**：使用编译的正则表达式模式，对性能影响最小化，仅在检测到ANSI字符时进行处理。
- HttpExecutor：
  - 单请求：轻量级，适合简单调用。
...
- 系统命令执行卡顿：
  ...
  - 避免过长命令链导致缓冲区溢出。
- **ANSI转义序列问题**：
  - SystemCmd已内置ANSI过滤功能，如果仍有显示问题，检查输出是否经过多次处理；
  - 某些特殊终端控制码可能需要额外的处理逻辑；
  - 在Web界面中，确保CSS样式不会干扰文本显示。
...
## 结论
命令执行器系列通过统一接口与特性机制实现了高度可扩展的执行框架。FileHelper 作为首个实现，提供了强大的文件操作能力；SystemCmd、HttpExecutor、DatabaseExecutor 分别覆盖系统命令、HTTP 请求与数据库操作三大场景，配合异步流式结果与模板化配置，满足从简单调用到复杂工作流的多样化需求。**最新版本的SystemCmd增加了ANSI转义序列过滤功能，确保在各种界面环境中都能显示干净的文本输出**。结合 DI 注册与工厂反射，开发者可快速扩展新的执行器并融入现有体系。
...
- SystemCmd 批量命令与并行命令示例：[ShellTest.cs:12-45](file://Sylas.RemoteTasks.Test/SystemHelperTest/ShellTest.cs#L12-L45)
- SystemCmd ANSI过滤功能：[SystemCmd.cs:406-420](file://Sylas.RemoteTasks.Utils/CommandExecutor/SystemCmd.cs#L406-L420)
...
- [ShellTest.cs:12-45](file://Sylas.RemoteTasks.Test/SystemHelperTest/ShellTest.cs#L12-L45)
- [SystemCmd.cs:406-420](file://Sylas.RemoteTasks.Utils/CommandExecutor/SystemCmd.cs#L406-L420)
