...
## 更新摘要
**变更内容**
- 更新了依赖注入配置：AnythingService 构造函数从 IServiceScopeFactory 改为 IServiceProvider
- 移除了集中式执行器缓存机制
- 更新了执行器解析逻辑，改为每次执行时从 DI 容器创建新的执行器实例
- 更新了内存缓存策略，移除了执行器级别的缓存
...
**图表来源**
- [AnythingService.cs:28-36](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L28-L36)
- [HostsController.cs:17-19](file://Sylas.RemoteTasks.App/Controllers/HostsController.cs#L17-L19)
- [ICommandExecutor.cs:30-31](file://Sylas.RemoteTasks.Utils/CommandExecutor/ICommandExecutor.cs#L30-L31)
- [ExecutorAttribute.cs:18-22](file://Sylas.RemoteTasks.Utils/CommandExecutor/ExecutorAttribute.cs#L18-L22)
- [TmplHelper.cs:20-200](file://Sylas.RemoteTasks.Utils/Template/TmplHelper.cs#L20-L200)
- [anything.js:1-38](file://Sylas.RemoteTasks.App/wwwroot/js/anything.js#L1-L38)

**章节来源**
- [AnythingService.cs:17-38](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L17-L38)
- [HostsController.cs:17-19](file://Sylas.RemoteTasks.App/Controllers/HostsController.cs#L17-L19)
...
- ExecutorAttribute：执行器特性，用于从 IServiceProvider 获取已注册的执行器实例。
...
- [AnythingExecutor.cs:5-10](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingExecutor.cs#L5-L10)
...
## 架构总览
AnythingService 采用依赖注入装配仓储、缓存、HTTP 客户端、服务提供器与日志器；通过内存缓存加速 AnythingInfo 与执行器解析；通过模板引擎解析命令与属性；通过异步流式输出命令执行结果；支持本地与跨节点命令转发。
...
+GetCommandHandler(executorName,args,serviceProvider)
}
class ExecutorAttribute {
+GetExecutor(executorName,serviceProvider)
}
class TmplHelper {
+ResolveExpressionValue(expr,context)
}
AnythingService --> ICommandExecutor : "动态创建执行器"
AnythingService --> ExecutorAttribute : "特性解析"
AnythingService --> TmplHelper : "解析模板"
HostsController --> AnythingService : "调用"
```

**图表来源**
- [AnythingService.cs:43-679](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L43-L679)
- [HostsController.cs:30-232](file://Sylas.RemoteTasks.App/Controllers/HostsController.cs#L30-L232)
- [ICommandExecutor.cs:30-31](file://Sylas.RemoteTasks.Utils/CommandExecutor/ICommandExecutor.cs#L30-L31)
- [ExecutorAttribute.cs:18-22](file://Sylas.RemoteTasks.Utils/CommandExecutor/ExecutorAttribute.cs#L18-L22)
- [TmplHelper.cs:20-200](file://Sylas.RemoteTasks.Utils/Template/TmplHelper.cs#L20-L200)
...
- 缓存与 HTTP：IMemoryCache、IHttpClientFactory、IHttpContextAccessor、IServiceProvider 注入。
...
Program->>DI : AddSingleton<IHttpRequestPipeline>()
Program->>DI : AddTransient<DataHandlerSyncDataToDb>()
Program->>DI : AddTransient<DataHandlerCreateTable>()
Program->>DI : AddTransient<DataHandlerAnonymization>()
Program->>DI : AddSingleton<DatabaseProvider>()
CTRL->>DI : 解析 AnythingService
SVC->>DI : 解析 RepositoryBase<AnythingSetting>
SVC->>DI : 解析 IMemoryCache/IHttpClientFactory/IServiceProvider
```

**图表来源**
- [Program.cs:48-64](file://Sylas.RemoteTasks.App/Program.cs#L48-L64)
- [HostsController.cs:17-19](file://Sylas.RemoteTasks.App/Controllers/HostsController.cs#L17-L19)
- [AnythingService.cs:28-36](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L28-L36)
...
- 执行器缓存：**已移除** 集中式执行器缓存，改为按需查询并缓存执行器元数据。
- 缓存更新策略：
...
subgraph "执行器缓存(已移除)"
LoadExec["查询执行器元数据"] --> CacheExec["缓存执行器元数据(1分钟)"]
end
```

**图表来源**
- [AnythingService.cs:254-276](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L254-L276)
- [AnythingService.cs:505-526](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L505-L526)
- [AnythingService.cs:541-556](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L541-L556)

**章节来源**
- [AnythingService.cs:247-276](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L247-L276)
- [AnythingService.cs:124-142](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L124-L142)
- [AnythingService.cs:177-196](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L177-L196)
- [AnythingService.cs:228-245](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L228-L245)
- [AnythingService.cs:541-556](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L541-L556)
...
alt 命令Domain=当前节点
SVC->>EXEC : 调用执行器(GetCommandHandler)
loop 异步流式输出
...
- [AnythingService.cs:293-394](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L293-L394)
- [anything.js:17-38](file://Sylas.RemoteTasks.App/wwwroot/js/anything.js#L17-L38)
...
**图表来源**
- [AnythingService.cs:82-105](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L82-L105)
- [AnythingService.cs:203-207](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L203-L207)
- [AnythingService.cs:93-105](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L93-L105)

**章节来源**
- [AnythingService.cs:39-105](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L39-L105)
- [HostsController.cs:162-214](file://Sylas.RemoteTasks.App/Controllers/HostsController.cs#L162-L214)

### AnythingInfo 构建过程
- 属性解析：从 AnythingSetting.Properties 解析为字典，补充默认路径常量等。
- 执行器解析：根据 AnythingExecutor.Name 与 Arguments（JSON 序列化）解析参数，支持类型转换。
- 执行器映射：通过 ICommandExecutor.GetCommandHandler 动态创建执行器，并使用 IServiceProvider 进行依赖注入。
...
ExecSel --> |否| LoadExec["从仓储加载执行器元数据"]
LoadExec --> Args["解析 Arguments 参数模板"]
Args --> CreateExec["动态创建执行器(GetCommandHandler)"]
CreateExec --> Map["执行器映射(使用IServiceProvider)"]
Map --> Cmds["逐条解析命令模板"]
...
**图表来源**
- [AnythingService.cs:534-632](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L534-L632)
- [AnythingService.cs:599-618](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L599-L618)
- [ICommandExecutor.cs:30-31](file://Sylas.RemoteTasks.Utils/CommandExecutor/ICommandExecutor.cs#L30-L31)
- [ExecutorAttribute.cs:18-22](file://Sylas.RemoteTasks.Utils/CommandExecutor/ExecutorAttribute.cs#L18-L22)
- [TmplHelper.cs:20-200](file://Sylas.RemoteTasks.Utils/Template/TmplHelper.cs#L20-L200)
...
**图表来源**
- [AnythingService.cs:55-75](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L55-L75)
- [AnythingService.cs:149-167](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L149-L167)
- [AnythingService.cs:541-556](file://Sylas.RemoteTasks.App/RemoteHostModule/Anything/AnythingService.cs#L541-L556)
...
- AnythingService 依赖：
  ...
  - 服务提供器：IServiceProvider
  - 日志：ILogger<AnythingService>
- 与 ICommandExecutor 的关系：通过反射与特性创建具体执行器实例。
- 与 ExecutorAttribute 的关系：通过特性从 IServiceProvider 获取已注册的执行器实例。
...
- 缓存策略
  ...
  - **执行器缓存：已移除集中式缓存，改为按需查询执行器元数据并缓存 1 分钟**。
- 异步与流式
...
## 结论
AnythingService 通过清晰的职责划分与完善的缓存策略，实现了配置驱动的命令执行体系。**最新的变更将 IServiceScopeFactory 替换为 IServiceProvider，移除了集中式执行器缓存，改为每次执行时从 DI 容器创建新的执行器实例**，这提高了执行器生命周期管理的灵活性。结合模板解析、异步流式输出与远程转发机制，能够高效支撑多节点的远程主机配置管理场景。建议在生产环境中进一步完善监控与告警，持续优化缓存命中率与执行器生命周期管理。
