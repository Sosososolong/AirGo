...
## 更新摘要
**所做更改**
- 新增自动化文件修改系统章节，详细介绍配置驱动的文件操作流水线
- 新增数据库实体代码生成功能说明，包括表结构分析和代码生成
- 新增模板引擎集成章节，涵盖 RazorEngine 和 TmplHelper 的使用
- 新增变量解析系统详解，包括全局变量、函数变量和条件语句
- 更新架构总览图，反映新增的模板引擎和变量解析组件
- 增强实际使用示例，展示完整的自动化文件修改流程
...
**更新** 本次更新重点增强了自动化文件修改系统，新增数据库实体代码生成、模板引擎集成和变量解析系统等功能，大幅提升了文件操作的智能化和自动化水平。
...
**图表来源**
- [Sylas.RemoteTasks.Utils/CommandExecutor/FileHelper.cs:1-1679](file://Sylas.RemoteTasks.Utils/CommandExecutor/FileHelper.cs#L1-L1679)
- [Sylas.RemoteTasks.Utils/Constants/SpaceConstants.cs:1-108](file://Sylas.RemoteTasks.Utils/Constants/SpaceConstants.cs#L1-L108)
- [Sylas.RemoteTasks.Utils/Constants/PathConstants.cs:1-25](file://Sylas.RemoteTasks.Utils/Constants/PathConstants.cs#L1-L25)
...
- [Sylas.RemoteTasks.Test/FileOp/FileHelperTest.cs:1-742](file://Sylas.RemoteTasks.Test/FileOp/FileHelperTest.cs#L1-L742)
...
- **新增** 数据库实体代码生成：自动分析表结构，生成实体类代码和 DTO 类。
- **新增** 模板引擎集成：支持 RazorEngine 和 TmplHelper 双模板引擎。
- **新增** 变量解析系统：支持全局变量、函数变量、条件语句和字符串替换。

**章节来源**
- [Sylas.RemoteTasks.Utils/CommandExecutor/FileHelper.cs:33-1679](file://Sylas.RemoteTasks.Utils/CommandExecutor/FileHelper.cs#L33-L1679)
...
## 架构总览
FileHelper 采用"配置驱动 + 模板变量 + 正则定位 + 批量修改"的架构模式，将文件操作抽象为"操作节点（Node）+ 步骤（Step）"的流水线，支持在工作目录内按正则匹配目标文件，再对每条目标文件执行一系列文本级操作。新增的模板引擎和变量解析系统进一步增强了自动化能力。
...
- [Sylas.RemoteTasks.Common/SecurityHelper.cs:1-209](file://Sylas.RemoteTasks.Common/SecurityHelper.cs#L1-L209)
- [Sylas.RemoteTasks.Utils/CommandExecutor/CommandResult.cs:1-38](file://Sylas.RemoteTasks.Utils/CommandExecutor/CommandResult.cs#L1-L38)
- [Sylas.RemoteTasks.Utils/Sylas.RemoteTasks.Utils.csproj](file://Sylas.RemoteTasks.Utils/Sylas.RemoteTasks.Utils.csproj#L26)
...
## 结论
FileHelper 通过"配置驱动 + 模板变量 + 正则定位 + 批量修改"的设计，将复杂的文件系统操作抽象为可维护、可扩展的流水线。结合编码检测、Base64/AES 安全能力与跨平台路径处理，能够高效应对配置管理、日志处理与备份等实际场景。新增的自动化文件修改系统、数据库实体代码生成、模板引擎集成和变量解析系统等功能，进一步提升了工具的智能化和自动化水平，建议在生产环境中配合异步 I/O、流式处理与严格的权限控制，确保稳定性与性能。
