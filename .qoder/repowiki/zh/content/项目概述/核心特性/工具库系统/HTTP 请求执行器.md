...
- **更新** HttpRequestSpec 模型已更新，明确 TimeoutSeconds 参数的特殊值含义（-1 表示无限超时用于 AI 请求）
...
**更新** 本次更新重点介绍了重构后的 HTTP 请求管道系统，该系统通过依赖注入 IHttpRequestPipeline 接口，提供了更好的分离关注点、增强的可配置性和可扩展性，替代了原有的直接 HTTP 客户端使用方式。最新更新明确了 TimeoutSeconds 参数的特殊值含义，-1 表示无限超时，专门用于 AI/长耗时请求场景。
...
- 字段说明
  ...
  - TimeoutSeconds：请求超时时间（秒）。**更新**：0 或未配置时使用管道默认 60 秒；-1 表示无限超时，专门用于 AI/长耗时请求。
  - Copy：浅拷贝当前对象。
- 使用场景
...
### 超时配置
- **更新** HttpRequestPipeline 使用 HttpClient.Timeout 设置超时，支持通过 HttpRequestSpec.TimeoutSeconds 配置。
- **更新** 特殊值处理：当 TimeoutSeconds 为 -1 时，设置为无限超时（Timeout.InfiniteTimeSpan），专门用于 AI/长耗时请求场景。
- **更新** 默认行为：当 TimeoutSeconds 为 0 或未配置时，使用 60 秒作为默认超时。
...
#### 超时处理机制
**更新** 详细的超时处理逻辑：
- 当 TimeoutSeconds = -1 时，设置为无限超时（Timeout.InfiniteTimeSpan）
- 当 TimeoutSeconds > 0 时，设置为指定秒数的超时
- 当 TimeoutSeconds = 0 或未配置时，设置为 60 秒默认超时
...
- **更新**：TimeoutSeconds 字段明确支持特殊值 -1 表示无限超时
...
- **更新** 无限超时优化：对于 AI/长耗时请求，使用 -1 特殊值设置无限超时，避免意外中断。
...
**更新** 新增了 HTTP 请求管道系统的性能考量，强调了管道系统在资源管理方面的优势和无限超时的优化。
...
- 常见错误
  ...
  - **新增** 超时问题：检查 TimeoutSeconds 配置，特别是 -1 无限超时的使用场景。
- 建议排查步骤
  ...
  - **新增** 验证 HttpRequestSpec 的配置是否符合预期，特别是 TimeoutSeconds 的特殊值处理。
  - **新增** 对于 AI/长耗时请求，确认使用 -1 无限超时配置是否正确。

**更新** 新增了 HTTP 请求管道系统的故障排查指导，包括超时问题和无限超时配置的检查。
...
## 结论
HttpExecutor 以简洁的命令模型与强大的模板解析能力，实现了从单请求到多线程并发、从简单到复杂的 HTTP 请求编排。通过依赖注入 IHttpRequestPipeline 接口，既保证了可维护性，也为性能优化与扩展提供了空间。最新的更新引入了全新的 HTTP 请求管道系统，该系统提供了更好的分离关注点、增强的可配置性和可扩展性，替代了原有的直接 HTTP 客户端使用方式。最新的更新明确了 TimeoutSeconds 参数的特殊值含义，-1 表示无限超时，专门用于 AI/长耗时请求场景，移除了不必要的显式资源释放操作，采用了更现代的资源管理模式，同时优化了响应处理机制，提高了系统的灵活性和安全性。建议在生产环境中结合超时、重试、限流与监控策略，确保稳定与高效。

**更新** 强调了 HTTP 请求管道系统带来的架构改进，以及资源管理和响应处理机制的优化，特别突出了无限超时配置的使用场景。
...
- 使用建议
  ...
  - **新增** 对于 AI/长耗时请求，使用 -1 特殊值配置无限超时，避免意外中断。
- 参考测试
