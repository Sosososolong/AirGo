...
namespace DemonFox.Tails.Infrastructure
{
    ...
    {           
        ...
        public async Task<string> InitialProjectAsync()
        {
            ...
            FileOp.DeleteFiles(CurrentProject.SolutionFiles.Where(f => f.EndsWith("Class1.cs")).Select(f=>f).ToList());
        ...
        public List<string> GetAspNetCoreWebDirectory(string dir)
        {
            ...
            List<string> webAppDirsMaybe = FileOp.GetDirectorysRecursive(FileOp.GetSolutionDirectory(),
            ...
            foreach (var dirItem in webAppDirsMaybe)
            {
                ...
                List<string> controllerDir = FileOp.GetDirectorysRecursive(dirItem, d => d.EndsWith(@"\Controllers") && Directory.GetFiles(d).AsQueryable().Where(f => f.EndsWith("Controller.cs")).Count() > 0);
        ...
        {
            // 找到文件
            // string propertyMappingServiceFile = FileOp.GetFilesRecursive(destinationUIDir, f => f.EndsWith(@"\PropertyMappingService.cs"), ls => ls.Count > 0).FirstOrDefault();            
            if (!File.Exists(propertyMappingServiceFile))
            {
                throw new ArgumentNullException(nameof(propertyMappingServiceFile));
            }
            //FileOp.InsertContent(propertyMappingServiceFile, originCode =>
            //{
            ...
            if (FileOp.IsContentExists(propertyMappingServiceFile, codeStart))
            {
        ...
        public OperationResult AddNewProp(string dtoParametersFile, string propTypeString, string propName, string defaultOrderByField = "")
        {
            ...
            List<string> dtoProps = FileOp.GetProperties(dtoParametersFile);
            ...
            FileOp.InsertContent(dtoParametersFile, originCode =>
            {
        ...
        public OperationResult AddServiceToDIContainer(string startupFile, string iserviceName, string serviceName)
        {
            ...
            if (FileOp.IsContentExists(startupFile, codeInserting))
            {
                return new OperationResult(false, $"在Startup文件中发现了代码{codeInserting}, 说明服务{iserviceName}已经注册过了");
            }
            FileOp.InsertContent(startupFile, originCode =>
            {
                return GetNewCode(originCode,
                    @"\r\n(\s+)services\..+;\r\n(\s+)}",
                    codeInserting,
        ...
        public OperationResult AddCodesAtMethodEnd(string file, string matchPattern, string addServiceCodes)
        {
            ...
            FileOp.InsertContent(file, originCode =>
            {
        ...
        public OperationResult AddDbResetCode(string programFile)
        {
            ...
            // db reset end
";
            if (FileOp.IsContentExists(programFile, "dbContext.Database.Migrate();"))
            {
                return new OperationResult(false, $"在Program.cs中已经发现了类似数据库迁移的代码");
            }
            FileOp.InsertContent(programFile, originCode =>
            {
                return GetNewCode(originCode,
        ...
        public string GetDtoToEntityPropertiesMappingDicItemsStatement(string dtoName, string entityName, string dtoProjectDir, string entityProjectDir)
        {
            ...
            string dtoFile = FileOp.FindFilesRecursive(dtoProjectDir, f => f.EndsWith($"{dto}.cs"), ls => ls.Count > 0).FirstOrDefault();
            string entityFile = FileOp.FindFilesRecursive(entityProjectDir, f => f.EndsWith($"{entity}.cs"), ls => ls.Count > 0).FirstOrDefault();
            ...
            List<string> dtoProperties = FileOp.GetProperties(dtoFile);
            List<string> entityProperties = FileOp.GetProperties(entityFile);
        ...
        {
            List<string> dbContextFiles = FileOp.FindFilesRecursive(dbContextDir, f => f.EndsWith("DbContext.cs"));
